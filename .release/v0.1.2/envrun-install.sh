...
for command_name in awk cmp curl cut find flock grep mkdir mktemp mv node rm rmdir \
  sha256sum ssh-keygen tar tr uname wc; do
...
[ "$(wc -l < "$checksum_file" | tr -d ' ')" = "4" ] || \
  fail "release checksum file is malformed"
for release_file in envrun-linux.tar.gz envrun-install.sh envrun.spdx.json \
  release-signing-key.pub; do
